...
describe('loadPlugins — guard acknowledgement declarations', () => {
  test('aggregates declarations by tool name', async () => {
    const loadEntry: LoadPluginEntryFn = async (entry) => ({
      name: entry,
      version: undefined,
      source: entry,
      defined: {
        guardAcknowledgements: [{ key: `${entry}Ack`, tools: ['bash', `${entry}_tool`] }],
        plugin: async () => ({}),
      },
    })
    ...
    expect([...result.registry.guardAcknowledgements.entries()].map(([tool, keys]) => [tool, [...keys]])).toEqual([
  ...
  test('rejects duplicate keys within one plugin without retaining partial registrations', async () => {
    const loadEntry: LoadPluginEntryFn = async () => ({
      name: 'duplicate',
      version: undefined,
      source: 'duplicate',
      defined: {
        guardAcknowledgements: [
          { key: 'sameAck', tools: ['bash'] },
          { key: 'sameAck', tools: ['read'] },
        ],
        plugin: async () => ({ tools: { duplicate_tool: noopTool() } }),
      },
    })
    const cap = captureWarnings()
    let result
    try {
      result = await loadPlugins({ entries: ['duplicate'], agentDir: '/tmp', configsByName: {}, loadEntry })
    } finally {
      cap.restore()
    }

    expect(result.failedPlugins).toEqual([
      { entry: 'duplicate', phase: 'register', error: expect.stringContaining('duplicate guard acknowledgement key') },
    ])
    expect(result.registry.tools).toEqual([])
    expect(result.registry.guardAcknowledgements.get('bash')).toBeUndefined()
  })

  test('rejects a key already owned by another plugin', async () => {
    const loadEntry: LoadPluginEntryFn = async (entry) => ({
      name: entry,
      version: undefined,
      source: entry,
      defined: {
        guardAcknowledgements: [{ key: 'sharedAck', tools: [entry] }],
        plugin: async () => ({}),
      },
    })
    const cap = captureWarnings()
    let result
    try {
      result = await loadPlugins({ entries: ['owner', 'conflict'], agentDir: '/tmp', configsByName: {}, loadEntry })
    } finally {
      cap.restore()
    }

    expect(result.loadedPlugins.map((plugin) => plugin.name)).toEqual(['owner'])
    expect(result.failedPlugins).toEqual([
      { entry: 'conflict', phase: 'register', error: expect.stringContaining('already declared by plugin owner') },
    ])
    expect(result.registry.guardAcknowledgements.get('owner')).toEqual(new Set(['sharedAck']))
    expect(result.registry.guardAcknowledgements.get('conflict')).toBeUndefined()
  })

  test('rejects plugin declarations that conflict with a first-party acknowledgement', async () => {
    const loadEntry: LoadPluginEntryFn = async () => ({
      name: 'conflict',
      version: undefined,
      source: 'conflict',
      defined: {
        guardAcknowledgements: [{ key: 'imageReadRedirect', tools: ['plugin_reader'] }],
        plugin: async () => ({}),
      },
    })
    const cap = captureWarnings()
    let result
    try {
      result = await loadPlugins({ entries: ['conflict'], agentDir: '/tmp', configsByName: {}, loadEntry })
    } finally {
      cap.restore()
    }

    expect(result.loadedPlugins).toEqual([])
    expect(result.failedPlugins[0]).toMatchObject({ entry: 'conflict', phase: 'register' })
    expect(result.registry.guardAcknowledgements.get('read')).toEqual(new Set(['imageReadRedirect']))
    expect(result.registry.guardAcknowledgements.get('plugin_reader')).toBeUndefined()
  })
})
