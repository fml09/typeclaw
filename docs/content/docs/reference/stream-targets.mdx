...
3. checks per-payload coalescing via `inFlightKey(payload)` — matching keys deduplicate within `parentSessionId` when present, or process-wide for scheduler dispatch without a parent
