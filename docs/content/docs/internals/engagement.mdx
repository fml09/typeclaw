...
## Mid-turn delivery: queue vs steer

An engaged inbound that lands while a turn is already running has two delivery paths:

- **Queue (default).** `enqueue()` pushes onto `live.promptQueue`; the drain while-loop splices it as the next batch after the in-flight prompt resolves. Every per-turn composition runs — `composeTurnPrompt` wrapping, quote anchors, attachment snapshots, per-turn origin/role refresh.
- **Steer** (`channels.<adapter>.steer.enabled`). A plain-text inbound is parked in `live.steerBuffer` for a quiet window (`flushSteerBufferMidTurn`) and injected into the RUNNING turn via `session.steer()` — the pi SDK delivers it after the current assistant message's tool calls settle, before the next model call, composed by `composeSteerPrompt` under an H2 heading with normal author attribution. Steered authors are added to `live.currentTurnAuthorIds` so reply targeting sees them. This bypasses turn composition by design — which is exactly why attachments, reply-quotes, and github review rounds are excluded (`isSteerableInbound`) and why a non-steerable inbound flushes the buffer to `promptQueue` first (arrival-order preservation).

Which path an inbound takes is decided by `resolveDeliveryMode(key)` (`router.ts`), consulted by both the steer gate in `route()` and `steerQuietMsFor`. It reads, in order: the per-channel-key **delivery-mode override**, then `channels.<adapter>.steer.enabled`, then the default (`queue`). The override is `'steer' | 'queue'`, set from the channel with [`/steer` and `/queue`](/docs/reference/channel-adapters), and its absence — not a third enum value — is what "follow the adapter" means.

It is persisted in its own file, `<agentDir>/channels/delivery-modes.json` (`{version: 1, modes: {[keyId]: 'steer'|'queue'}}`, write-then-rename), loaded once at router construction and awaited in `route()` immediately before the gate. Deliberately NOT a field on `ChannelSessionRecord`: that record carries a migration chain keyed to session identity, while a delivery preference has nothing to do with which session a channel is bound to and outlives every session rollover.

The resolution also carries `quietMs` (the adapter's `steer.quietMs`, else `inboundBatching.quietMs`, else the hot-debounce cadence), because an override can enable steering on an adapter that declares no `steer` block at all — and then there is no `steer.quietMs` to read. `/steer <text>` passes a window of 0: the operator already said "now", so there is nothing left to coalesce, and anything already buffered rides along with it in arrival order. It runs the same `isSteerableInbound` check the passive gate runs, so prefixing a message with `/steer` never costs the user content the identical message would have kept without the prefix — a payload carrying an attachment, a reply-quote, or a review round falls through to the queued path, which composes all three.

The invariant both paths share: a message is never dropped. Every fallback — turn ended during the window (drain's prompt-finally `flushSteerBufferToQueue`), aborting turn, reload teardown, `session.steer()` rejection — lands the buffer on `promptQueue`, which the teardown carry hands to the successor session. `/stop` is the one dropper: it clears `steerBuffer` alongside the queued prompts it already cancels.
