...
- **`RuntimeCredentialRenewer`** — managed runtimes run the KakaoTalk renewal loop in-process against the writable Agent Folder and reload only the rotated adapter; host deployments retain the hostd-owned loop.
...
| `CredentialRenewer` (Webex/Teams crons) | hostd-owned                                        | extract when a managed renewal home is needed |
